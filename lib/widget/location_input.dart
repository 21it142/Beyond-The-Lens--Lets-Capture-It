...
class LocationInput extends StatefulWidget {
  ...
  final void Function(PalceLocation location) onSelectLocation;
...
class _LocationInputState extends State<LocationInput> {
  ...
  Future<void> _savePlace(double latitude, double longitude) async {
    final url = Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json?latlng=$latitude,$longitude&key=AIzaSyCxQKluusVfCZynsXyv-qVllkQNmJKTSN4');
    final response = await http.get(url);
    final resData = json.decode(response.body);
    final address = resData['results'][0]['formatted_address'];
    setState(() {
      _pickedLocation = PalceLocation(
        address: address,
        latitude: latitude,
        longitude: longitude,
      );
